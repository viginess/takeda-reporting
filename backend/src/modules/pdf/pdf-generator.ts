...
export async function generateSafetyPDF(report: any): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    ...
    // Title & Report ID
    doc.fillColor(clinRed).fontSize(22).text('Individual Case Safety Report (ICSR)', 50, 80);
    doc.fontSize(10).fillColor(slateGray).text(`Internal Report ID: ${report.id}`, 50, 105);
    doc.moveDown(0.5);

    // Horizontal Line
    doc.strokeColor(clinRed).lineWidth(1).moveTo(50, 125).lineTo(550, 125).stroke();
    doc.moveDown(2);

    // Section 1: Regulatory Metadata (N)
    drawSectionHeader(doc, 'SECTION 1: REGULATORY METADATA', clinRed);
    ...
    drawSectionHeader(doc, 'SECTION 1.5: PRIMARY REPORTER DETAILS', clinRed);
    ...
    drawSectionHeader(doc, 'SECTION 2: PATIENT CHARACTERISTICS', clinRed);
    ...
    drawSectionHeader(doc, 'SECTION 3: ADVERSE REACTIONS', clinRed);
    ...
    drawSectionHeader(doc, 'SECTION 4: SUSPECT DRUG INFORMATION', clinRed);
    ...
    if (report.hasRelevantHistory === 'yes' || report.labTestsPerformed === 'yes') {
        drawSectionHeader(doc, 'SECTION 5: RELEVANT HISTORY & LAB TESTS', clinRed);
    ...
    drawSectionHeader(doc, 'SECTION 6: CASE NARRATIVE & SENDER COMMENTS', clinRed);
