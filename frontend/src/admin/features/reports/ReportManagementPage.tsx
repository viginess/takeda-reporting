...
export default function ReportManagementPage() {
  ...
  const meddraResults = undefined as any;
  const searchingMeddra = false;
  ...
  const handleSave = () => {
    ...
    setSelectedReport(executeUpdate(selectedReport, {
  ...
  return (
    <Flex h="100%" direction="column" bg="#f8fafc" fontFamily="'DM Sans', system-ui, sans-serif">
      ...
      <Box pt={8} px={8}>
        ...
        {/* Filters */}
        <Flex as={motion.div as any} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.08 } as any} gap={3} mb={5} align="center">
          ...
          <Flex gap={1} bg="white" border="1px solid" borderColor="#e2e8f0" borderRadius="xl" p={1}>
            {["All", ...statusOptions].map(s => (
              <Button key={s} onClick={() => setFilterStatus(s)} size="sm" borderRadius="md" fontSize="xs"
          ...
          <Flex gap={1} bg="white" border="1px solid" borderColor="#e2e8f0" borderRadius="xl" p={1}>
            {["All", "Patient", "HCP", "Family"].map(t => (
              <Button key={t} onClick={() => setFilterType(t)} size="sm" borderRadius="md" fontSize="xs"
        ...
        <ModalContent bg="transparent" boxShadow="none">
          ...
          <ModalBody display="flex" alignItems="center" justifyContent="center" p={0} onClick={onZoomClose}>
            <Center w="full" h="full">
              <Image src={zoomedImage} maxH="90vh" maxW="90vw" objectFit="contain"
